...
export default function VisualEditAgent() {
	...
	const findElementsById = (id, _preferSourceLocation = false) => {
		if (!id) return [];
	...
	const handleElementClick = (e) => {
		...
		const elementData = {
			...
			classes: (element as any).className?.baseVal || (element as any).className || '',
	...
	const updateElementClasses = (visualSelectorId, classes, replace = false) => {
		...
		elements.forEach(element => {
			if (replace) {
				...
				const currentClasses = (element as any).className?.baseVal || (element as any).className || '';
