...
export default function NotificationCenter({ language }) {
    ...
    const loadNotifications = async () => {
        try {
            const data = await appClient.entities.FarmerNotification.list('-created_date', 20);
    ...
    const markAsRead = async (id) => {
        await appClient.entities.FarmerNotification.update(id, { is_read: true });
        setNotifications(prev => prev.map(n => n.id === id ? { ...n, is_read: true } : n));
    };

    const markAllAsRead = async () => {
        const unread = notifications.filter(n => !n.is_read);
        await Promise.all(unread.map(n => appClient.entities.FarmerNotification.update(n.id, { is_read: true })));
        setNotifications(prev => prev.map(n => ({ ...n, is_read: true })));
    };

    const deleteNotification = async (id) => {
        await appClient.entities.FarmerNotification.delete(id);
        setNotifications(prev => prev.filter(n => n.id !== id));
    };

    const clearAll = async () => {
        await Promise.all(notifications.map(n => appClient.entities.FarmerNotification.delete(n.id)));
        setNotifications([]);
    };
